...
class ListadoAutor(LoginYSuperStaffMixin,ValidarPermisosMixin,ListView):
	...
	def get(self,request,*args,**kwargs):
		if request.is_ajax():
			...
			data = {
				'lenght': self.get_queryset().count(),
				'objects':list_data
			}
	...
	def post(self, request, *args, **kwargs):
		if request.is_ajax():
			...
			if form.is_valid():
				...
				nuevo_autor.save()
				mensaje = f'{self.model.__name__} resgistrado correctamente!'
				error = 'No hay error!'
				response = JsonResponse({'mensaje': mensaje, 'error': error})
				response.status_code = 201
				return response
...
class EliminarAutor(LoginYSuperStaffMixin,ValidarPermisosMixin,DeleteView):
	...
	def delete(self,request,pk,*args,**kwargs):
		if request.is_ajax():
			...
			autor.estado = False
			autor.save()
			mensaje = f'{self.model__name__} eliminado correctamente!'
			error = 'No hay error!'
			response = JsonResponse({'mensaje': mensaje, 'error': error})
			response.status_code = 201
			return response
...
class ListadoLibros(LoginYSuperStaffMixin,ValidarPermisosMixin,ListView):
	...
	def get(self,request,*args,**kwargs):
		if request.is_ajax():
			return HttpResponse(serialize('json', self.get_queryset(),
				use_natural_foreign_keys=True), 'application/json')
		else:
			return redirect('libro:inicio_libro')
